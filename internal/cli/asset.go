...
Examples:
  harbor asset list --workspace "sql-for-research"
  harbor asset create style.css --workspace "sql-for-research" --body-file /tmp/style.css`,
}
...
var assetListCmd = &cobra.Command{
  ...
  User     — components authored with 'harbor asset create'.

Each row shows whether the asset is fully present and the command that acts
on it: 'harbor asset add' when absent, 'harbor asset redeploy' when present.`,
	Args: cobra.NoArgs,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		for _, n := range names {
			...
			hint := "harbor asset redeploy " + n
			if !p {
				hint = "harbor asset add " + n
			}
		...
		if len(seeded)+len(vendored)+len(user) == 0 {
			...
			fmt.Printf("  Use 'harbor asset create <filename> --body-file <path>' to add one.\n")
			fmt.Println()
		}
		return nil
	},
...
Examples:
  harbor asset create style.css --workspace "sql-for-research" --body-file /tmp/style.css
  harbor asset create quiz-widget.js --workspace "yoga" --body-file /tmp/widget.js`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if bodyFile == "" {
			return fmt.Errorf("--body-file is required\n  harbor asset create %q --workspace %q --body-file <path>", filename, ws.Name)
		}
