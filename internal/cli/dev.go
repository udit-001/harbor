...
// devFlags holds the flags for `harbor dev`.
var devFlags struct {
	port   int
	noOpen bool
}
...
// manages a child `harbor start --foreground --dev-css` process.
func runDev() error {
	root := mustProjectRoot()
	tailwindBin := filepath.Join(root, ".bin", "tailwindcss")
	devBin := filepath.Join(root, "tmp", "harbor-dev")
	tmpBin := devBin + ".next"

	if _, err := os.Stat(tailwindBin); err != nil {
		return fmt.Errorf("tailwind binary not found at %s — run `harbor dev` from the project root", tailwindBin)
	}
...
// buildBinary compiles ./cmd/harbor to out.
func buildBinary(root, out string) error {
	if err := runTemplGenerate(root); err != nil {
		return err
	}
	cmd := exec.Command("go", "build", "-o", out, "./cmd/harbor")
	cmd.Dir = root
...
// mustProjectRoot returns the CWD; `harbor dev` is run from the project root.
