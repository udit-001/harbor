...
import (
	...
	migrateutil "github.com/udit-001/harbor/internal/migrate"
)
...
Use 'harbor migrate up' to apply pending migrations,
'harbor migrate down' to roll back the last one, and
'harbor migrate status' to see the current state.

Examples:
  harbor migrate up
  harbor migrate down
  harbor migrate status
  harbor migrate up-to 1
  harbor migrate down-to 0`,
}
...
var migrateUpCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		rawDB, err := db.OpenRaw(filepath.Join(resolveDataDir(), "harbor.db"))
		if err != nil {
