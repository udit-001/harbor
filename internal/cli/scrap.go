...
Examples:
  harbor scrap list
  harbor scrap list --search "ml engineer"
  harbor scrap list --status done
  harbor scrap read ml-engineer
  harbor scrap add "ML engineer roadmap" --body-file /tmp/roadmap.md --tag ml --tag career
  harbor scrap update ml-engineer --body-file /tmp/roadmap.md --status active`,
}
...
Examples:
  harbor scrap read ml-engineer
  harbor scrap read ml-engineer --json`,
...
Examples:
  harbor scrap add "ML engineer roadmap" --body-file /tmp/roadmap.md --tag ml --tag career`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if bodyFile == "" {
			return fmt.Errorf("--body-file is required\n  harbor scrap add %q --body-file <path>", title)
		}
...
Examples:
  harbor scrap update ml-engineer --body-file /tmp/roadmap.md
  harbor scrap update ml-engineer --title "ML engineer" --status done
  harbor scrap update ml-engineer --tag ml --tag career`,
	Args: cobra.ExactArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		s := mustStore(cmd)
		slug := args[0]
...
Examples:
  harbor scrap delete ml-engineer`,
	Args: cobra.ExactArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		s := mustStore(cmd)
