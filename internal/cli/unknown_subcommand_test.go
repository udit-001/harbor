...
func TestParentCommandsRejectUnknownSubcommands(t *testing.T) {
	for _, p := range parentCommands {
		t.Run(p, func(t *testing.T) {
			...
			if err == nil {
				t.Errorf("harbor %s <unknown>: expected error, got nil", p)
			}
		})
	}
}
...
// succeed (print help) — i.e. the fix must not make `harbor skills` error.
func TestParentCommandsWithoutArgsShowHelp(t *testing.T) {
	for _, p := range parentCommands {
		t.Run(p, func(t *testing.T) {
			err := executeForTest(t, []string{p})
			if err != nil {
				t.Errorf("harbor %s: expected no error (help), got %v", p, err)
			}
		})
	}
}
