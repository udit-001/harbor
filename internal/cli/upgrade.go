...
var upgradeCmd = &cobra.Command{
	Use:   "upgrade",
	Short: "Upgrade harbor to the latest version via 'go install'",
	Long: `Upgrade harbor to the latest release by running:
...
restarted afterwards.`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err != nil {
			...
			fmt.Println("    go install github.com/udit-001/harbor/cmd/harbor@latest")
			fmt.Println()
			return nil
		}
		...
		module := fmt.Sprintf("github.com/udit-001/harbor/cmd/harbor@%s", latest)
		...
		if serverInfo != nil {
			...
			if err != nil {
				...
				fmt.Printf("  Run 'harbor start' manually.\n")
			} else {
				fmt.Printf("  Server restarted in background (PID: %d)\n", rc.Process.Pid)
				fmt.Printf("  http://127.0.0.1:%d\n", serverInfo.Port)
			}
		} else {
			fmt.Printf("  Run 'harbor start' to launch the server.\n")
		}
...
func latestVersionFromProxy(goPath string) (string, error) {
	cmd := exec.Command(goPath, "list", "-m", "-versions", "github.com/udit-001/harbor")
