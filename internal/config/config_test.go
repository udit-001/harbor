...
func TestLoadCorrupt(t *testing.T) {
	...
	cfgDir := filepath.Join(dir, "harbor")
	os.MkdirAll(cfgDir, 0755)
	os.WriteFile(filepath.Join(cfgDir, "harbor.toml"), []byte("not valid toml {{{"), 0644)
...
func TestSaveExpandsTilde(t *testing.T) {
	...
	cfg := &Config{DataDir: "~/my-harbor"}
	if err := Save(cfg); err != nil {
		t.Fatalf("Save: %v", err)
	}
	if cfg.DataDir != filepath.Join(home, "my-harbor") {
		t.Fatalf("expected %s, got %s", filepath.Join(home, "my-harbor"), cfg.DataDir)
	}
}
...
func TestPaths(t *testing.T) {
	...
	if filepath.Base(p) != "harbor.toml" {
		t.Fatalf("Path() should end with harbor.toml: %s", p)
	}
	...
	cd := ConfigDir()
	if filepath.Base(cd) != "harbor" {
		t.Fatalf("ConfigDir() should end with harbor: %s", cd)
	}
}
