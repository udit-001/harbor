...
func Start(cfg Config) error {
	...
	if err != nil {
		return fmt.Errorf("port %d is already in use. Free it, or set a different port: `harbor config set port <N>`", cfg.Port)
	}
	...
	// so `harbor start`/`harbor dev` don't print a spurious error on stop.
